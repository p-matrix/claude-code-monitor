...
- **Migrate to `@pmatrix/core-sdk@^0.1.0`** — `PMatrixHttpClient` 와 공통 schema (5-layer Agent Host-Integration Adapter Contract v0.1)를 `@pmatrix/core-sdk` 로 위임. SDK 자체 substance ~437 LOC 감소, API surface 변경 0 (consumer 영향 없음).

### Verification

- Fresh install: `node_modules/@pmatrix/core-sdk` 가 registry tarball 추출 confirmed (symlink=false / version=0.1.0)
- Tests: 156 PASS, regression 0 (R-X.4 lockstep verify, monorepo commit `d1a6d08`)
