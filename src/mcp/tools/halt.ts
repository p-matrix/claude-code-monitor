// =============================================================================
// @pmatrix/claude-code-monitor — mcp/tools/halt.ts
// pmatrix_halt MCP tool
//
// Global Kill Switch: immediately blocks all tool execution across all sessions.
//
// Mechanism:
//   1. Create ~/.pmatrix/HALT file (checked by PreToolUse / PermissionRequest hooks)
//   2. Send stability=1.0 critical signal to server (best-effort)
//
// Resume: rm ~/.pmatrix/HALT
//
// ⚠️ This is a GLOBAL halt — affects all Claude Code sessions on this machine.
...
export async function handleHaltTool(
  ...
  lines.push('');
  lines.push('All Claude Code sessions on this machine are now blocked.');
