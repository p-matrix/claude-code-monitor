// =============================================================================
// @pmatrix/claude-code-monitor — mcp/server.ts
...
const TOOLS: Tool[] = [
  {
    name: 'pmatrix_status',
    description:
      'Show current P-MATRIX safety grade, R(t) risk score, mode, and session counters for this Claude Code session.',
    inputSchema: {
      type: 'object',
      properties: {
        session_id: {
  ...
  },
  {
    name: 'pmatrix_halt',
    description:
      '⛔ HALT — immediately block all tool execution across all Claude Code sessions. Creates ~/.pmatrix/HALT. To resume: rm ~/.pmatrix/HALT',
    inputSchema: {
      type: 'object',
      properties: {
        reason: {
...
/**
 * Find the most recently modified field-*.json file in sessions dir.
 * Returns sessionKey (without "field-" prefix and ".json" suffix) or null.
 */
...
/**
 * Start IPC poller that reads field state from hooks and sends State Vectors.
 */
  ...
  setInterval(() => {
    try {
      ...
      if (state.axes) {
        ...
        // Degraded SV: CC 3.5 설계상 개별 축 값 미보유
      ...
      // Fail-open: poller errors must not crash MCP server
    }
...
export async function runMcpServer(): Promise<void> {
  ...
  // Start IPC poller after MCP transport is connected
  if (fieldNode) {
